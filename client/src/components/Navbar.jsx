...
import { Menu, X, User } from "lucide-react"; // ✅ User icon
...
export default function Navbar() {
  ...
  const [showDashboard, setShowDashboard] = useState(false); // ✅ for dropdown
  ...
  // Choose font class based on language
  const fontClass =
      ...
      <div className="max-w-screen-2xl mx-auto px-6 lg:px-12">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-4 relative">
            ...
            {user ? (
              <>
                {/* ✅ User icon instead of Dashboard text */}
                <button
            ...
            {user ? (
              <>
                ...
                {showDashboard && (
                  <div
                    ref={dashboardRef}
                    className="mt-2 bg-[#f9fafa] border border-[#d5d8d9] rounded-lg shadow-lg z-50"
                  >
                    <Dashboard user={user} onLogout={logout} />
                  </div>
                )}
